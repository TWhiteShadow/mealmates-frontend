...
const UserPage = () => {
  ...
  if (error || !user) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-red-500">Impossible de charger les données de cet utilisateur</p>
      </div>
    );
  }
      ...
      <div className="max-w-md mx-auto px-4">
        <section className="my-8">
          <div className="grid grid-cols-2 gap-4">
            <StatCard
              title="Note moyenne"
              value={!isLoading && user?.averageRating ? user.averageRating.toFixed(2) : "0.0"}
              unit="/5"
              isLoading={isLoading}
          ...
          {isLoadingOffers ? (
            ...
            <div className="space-y-4">
              {userOffers && userOffers.length > 0 ? (
                userOffers.slice(0, 3).map((product) => (
