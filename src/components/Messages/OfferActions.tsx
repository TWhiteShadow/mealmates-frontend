...
import {
  ...
  useGeneratedQRCodeToken,
  useReserveProductMutation,
  useTransactionPaymentLink,
  validateQRCode,
} from '@/api/Paiement';
...
const ProductActions = ({
  ...
  const redirectURI =
    location.pathname + (selectedId ? `?conversation=${selectedId}` : '');

  const lastTransaction =
    (transactions && transactions[transactions.length - 1]) || null;
  ...
  const hasToPay = useMemo(
    () =>
      ...
      !isFree &&
      otherParticipant &&
      otherParticipant.id === product.seller.id,
    [
      isSeller,
      isBuyer,
      hasBuyer,
  ...
  const hasReviewed = useMemo(
    () =>
      lastTransaction &&
      lastTransaction.reviews?.some(
        (review) =>
          review.reviewer.id === userData?.data?.id &&
  ...
  const { data: paymentLink, isLoading } = useTransactionPaymentLink(
    lastTransaction?.id || 0,
    redirectURI,
    hasToPay
  );
  const { data: qrCodeUrl } = useGeneratedQRCodeToken(
    lastTransaction?.id || 0,
    generateQRCode
  );
  ...
  if (!isSeller && !isBuyer && hasBuyer) {
    if (isReserved || isConfirmed || isPending) {
      return (
        <ActionBanner
          title='Ce produit est déjà réservé'
          description='Ce produit est actuellement réservé par un autre utilisateur.'
        />
      );
    } else if (isCompleted) {
      return (
        <ActionBanner
          title='Ce produit a été vendu'
          description='La transaction pour ce produit est déjà terminée.'
        />
      );
    }
  }
  ...
  if (
    ...
    isReserved &&
    otherParticipant &&
    otherParticipant.id === product.buyer?.id
  ) {
    return (
      <ActionBanner
        ...
        <div className='textBox text-center'>
          {!isFree ? (
            <p className='text-gray-600'>
              Merci d'avoir payer pour ce produit !
            </p>
          ) : (
            <p className='text-gray-600'>
              Super, vous pouvez maintenant aller chercher le produit !
            </p>
          )}
          <p className='text-gray-600'>
            Au moment de votre rencontre, le vendeur devra scanner le QR code
            ci-dessous.
          </p>
        </div>
    ...
    return (
      <div className='flex gap-5 flex-wrap min-h-16 p-4 items-center justify-center bg-white border-t border-gray-200'>
        <div className='textBox text-center'>
          {!isFree ? (
            <p className='text-gray-600'>
              {otherParticipant.first_name} {otherParticipant.last_name} à payer
              pour le produit.
            </p>
          ) : (
            <p className='text-gray-600'>
              {otherParticipant.first_name} {otherParticipant.last_name}{' '}
              souhaite récupérer le produit !
            </p>
          )}
          <p className='text-gray-600'>
            Vous devez maintenant scanner son QR Code.
          </p>
        </div>
    ...
    return (
      <>
        <ActionBanner
          title={
            !isFree
              ? `Merci d'avoir vendu votre produit !`
              : `Merci d'avoir donné votre produit !`
          }
  ...
  if (hasBuyer && isCompleted && hasReviewed && (isBuyer || isSeller)) {
    return (
      <>
        <ActionBanner title={`Merci d'avoir laissé un avis !`} />
      </>
    );
  }
};
